...
def test_socks5_proxy(server, port):
    ...
    try:
        ...
        if not method_res or method_res != 0x02 and method_res != 0x00:
            sock.close()
            return port, False
        ...
        if method_res == 0x02:
            ...
            if not auth_res or auth_res != 0x00:
                sock.close()
                return port, False
        ...
        is_valid = conn_res and conn_res == 0x00
...
def main():
    ...
    msg_header = (
        f"⏰ 【双重检测任务】扫描验证完成！\n"
        f"🌐 目标主机: {target_ip}\n"
        f"📊 统计数据: **本次共筛查出 {len(valid_proxies)} 个可用代理**（全量开放端口共 {len(open_ports)} 个）\n\n"
    )
